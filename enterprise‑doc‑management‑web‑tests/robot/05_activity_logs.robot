...
Select Filter Function
    ...
    # รอจนกว่า ${locator_input_filter} จะปรากฎ
    ...
    # สร้างเงื่อนไขหาก Locator = ${no_data_visible} ให้ทำ IF หากไม่ใช่ให้ทำ ELSE
    IF    ${no_data_visible}
        # ตรวจสอบ Element ว่าแสดงขึ้นมาจริง
        Element Should Be Visible          ${locator_no_data_message}
        ${message}=    Get Text            ${locator_no_data_message}
        # ตรวจสอบ ${message} = ค่าที่คาดหวังไหม
        Should Be Equal    ${message}      ไม่พบข้อมูลที่ต้องการค้นหา!
    ELSE
        ${locator_data_table_function}=    Set Variable    xpath=//div[contains(text(),'${text}')]
        # รอจนกว่า Element จะปรากฎ โดยสร้างเงื่อนไขให้รอแค่ 5s
